...
from src_1.util.files.file_handler import File_Handler
from src_1.util.files.file_io_helper import File_IO_Helper
from src_1.util.helpers import Safe_Datetime_Converter


class To_Download_List_Creator():
    ...
    def create_timestamp_list(start_time_stamp, current_time_stamp, max_to_be_downloaded_amount):
        ...
        for increase in range(to_be_downloaded):
            ...
            from_time_stamp_str = Safe_Datetime_Converter.datetime_to_string(from_time_stamp)
            to_time_stamp_str = Safe_Datetime_Converter.datetime_to_string(to_time_stamp)

            time_stamp_str_list.append((from_time_stamp_str, to_time_stamp_str))
            from_time_stamp = to_time_stamp + timedelta(microseconds=1)

        next_start_time_stamp = Safe_Datetime_Converter.string_to_datetime(time_stamp_str_list[-1][1]) + timedelta(microseconds=1)
        if to_be_downloaded == max_to_be_downloaded_amount:
            not_caught_up_with_now = True
        else:
            not_caught_up_with_now = False

        next_start_time_stamp_str = Safe_Datetime_Converter.datetime_to_string(next_start_time_stamp)
    ...
    def create_missing_order_id_list():
        ...
        if os.path.isfile(last_downloaded_missing_order_id_file_path):
            last_downloaded_missing_order_id = File_IO_Helper.read_last_downloaded_missing_order_id()
    ...
    def create_timestamp_list_missing_values(get_type_string, status_string):
        ...
        to_check_timestamp_folders_path = File_Handler.get_base_path("to_check_timestamp_folders")
