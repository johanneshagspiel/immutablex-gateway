...
from scrappers.gods_unchained_scrapper import Gods_Unchained_Scrapper
...
from util.helpers import Panda_Helper
...
class Data_Creator():
    ...
    def parallel_process_downloaded_info(get_type_string, active_updated_orders_df):
        ...
        if filled_orders_to_be_processed_exist:
            filled_orders_to_be_processed_order_list = Order_Administrator_GU.create_order_list_from_path_list(filled_orders_to_be_processed_order_path_list)
    ...
    def create_info_for_active_orders(sell_order_df, currency_1_str, currency_2_str, number_same_cards):
        ...
        storage_path = File_Handler.get_base_path("potential_purchases")

        last_24_hours_sales_path = File_Handler.get_base_path("last_24_hours_sales")
        last_7_days_sales_path = File_Handler.get_base_path("last_7_days_sales")
        last_30_days_sales_path = File_Handler.get_base_path("last_30_days_sales")

        last_24_hours_price_path = File_Handler.get_base_path("last_24_hours_price")
        last_7_days_price_path = File_Handler.get_base_path("last_7_days_price")
        last_30_days_price_path = File_Handler.get_base_path("last_30_days_price")

        last_24_hours_price_change_path = File_Handler.get_base_path("last_24_hours_price_change")
        last_7_days_price_change_path = File_Handler.get_base_path("last_7_days_price_change")
        last_30_days_price_change_path = File_Handler.get_base_path("last_30_days_price_change")

        last_30_days_total_win_rate_path = File_Handler.get_base_path("last_30_days_total_win_rate_file")
        last_30_days_mythic_win_rate_path = File_Handler.get_base_path("last_30_days_mythic_win_rate_file")
        last_4_weekend_rank_total_win_rate_path = File_Handler.get_base_path("last_4_weekend_rank_total_win_rate_file")
        last_4_weekend_rank_mythic_win_rate_path = File_Handler.get_base_path("last_4_weekend_rank_mythic_win_rate_file")

        bot_info_path = File_Handler.get_base_path("bot_info_dic")
        ...
        for currency_combination in currency_combination_list:
            ...
            for card_quality in quality_list:
                ...
                index_min_currency_1_list = currency_1_df.groupby("card_name")["price_euro"].nsmallest(number_same_cards).index.to_list()
                ...
                min_currency_1_df = min_currency_1_df[["card_name", "price_euro", "price_crypto", "token_id", "order_id"]]
                min_currency_1_df = min_currency_1_df.rename(columns={"price_euro": currency_1_cheapest_euro, "price_crypto" : currency_1_cheapest, "token_id": currency_1_token_id, "order_id" : currency_1_order_id})
                ...
                min_currency_2_df = min_currency_2_df[["card_name", "price_euro", "price_crypto", "token_id", "order_id"]]
                min_currency_2_df = min_currency_2_df.rename(columns={"price_euro": currency_2_cheapest_euro, "price_crypto" : currency_2_cheapest, "token_id": currency_2_token_id, "order_id" : currency_2_order_id})
                min_currency_2_df = min_currency_2_df.set_index("card_name")

                bot_df = currency_2_df.groupby("card_name")["user"].apply(lambda x: Panda_Helper.filtering_bot_user_series(x, bot_info_dic)).to_frame()
                ...
                combined_df_currency_1["t_dif"] = combined_df_currency_1[currency_2_cheapest_euro] - combined_df_currency_1[currency_1_cheapest_euro]
                combined_df_currency_1["rel_dif"] = (combined_df_currency_1["t_dif"] / combined_df_currency_1[currency_1_cheapest_euro]) * 100
                combined_df_currency_1["quality"] = card_quality

                combined_df_currency_1 = combined_df_currency_1.dropna(subset=[currency_2_cheapest_euro, currency_1_cheapest_euro])
                combined_df_currency_1["card_name"] = combined_df_currency_1["card_name"].replace({"Oddi, Valka’s Herald" : "Oddi, Valka's Herald"})

                currency_specific_24_hours_sales_path = str(last_24_hours_sales_path) + "\last_24_hours_sales_" + currency_2_name + "_" + card_quality + ".json"
                with open(currency_specific_24_hours_sales_path, 'r', encoding='utf-8') as currency_specific_24_hours_sales_file:
                    last_24_hours_sale_dic = json.load(currency_specific_24_hours_sales_file)
                currency_specific_24_hours_sales_file.close()

                currency_specific_7_days_sales_path = str(last_7_days_sales_path) + "\last_7_days_sales_" + currency_2_name + "_" + card_quality + ".json"
                with open(currency_specific_7_days_sales_path, 'r', encoding='utf-8') as currency_specific_7_days_sales_file:
                    last_7_days_sale_dic = json.load(currency_specific_7_days_sales_file)
                currency_specific_7_days_sales_file.close()

                currency_specific_30_days_sales_path = str(last_30_days_sales_path) + "\last_30_days_sales_" + currency_2_name + "_" + card_quality + ".json"
                with open(currency_specific_30_days_sales_path, 'r', encoding='utf-8') as currency_specific_30_days_sales_file:
                    last_30_days_sale_dic = json.load(currency_specific_30_days_sales_file)
                currency_specific_30_days_sales_file.close()


                currency_specific_24_hours_price_path = str(last_24_hours_price_path) + "\last_24_hours_price_" + currency_2_name + "_" + card_quality + ".json"
                ...
                currency_specific_7_days_price_path = str(last_7_days_price_path) + "\last_7_days_price_" + currency_2_name + "_" + card_quality + ".json"
                with open(currency_specific_7_days_price_path, 'r', encoding='utf-8') as currency_specific_7_days_price_file:
                    last_7_days_price_dic = json.load(currency_specific_7_days_price_file)
                currency_specific_7_days_price_file.close()

                currency_specific_30_days_price_path = str(last_30_days_price_path) + "\last_30_days_price_" + currency_2_name + "_" + card_quality + ".json"
                with open(currency_specific_30_days_price_path, 'r', encoding='utf-8') as currency_specific_30_days_price_file:
                    last_30_days_price_dic = json.load(currency_specific_30_days_price_file)
                currency_specific_30_days_price_file.close()


                currency_specific_24_hours_price_change_path = str(last_24_hours_price_change_path) + "\last_24_hours_price_change_" + currency_2_name + "_" + card_quality + ".json"
                ...
                combined_df_currency_1[last_7_days_at_least_one_sale] = combined_df_currency_1[last_7_days_sales].apply(lambda x: Panda_Helper.days_in_last_x_with_at_least_y_sales(x, 8, 1))
                combined_df_currency_1[last_7_days_sales] = combined_df_currency_1[last_7_days_sales].apply(lambda x: Panda_Helper.total_sales_in_the_past(x))

                combined_df_currency_1[last_30_days_sales] = combined_df_currency_1["card_name"].apply(lambda x: last_30_days_sale_dic[x])
                combined_df_currency_1[last_30_days_at_least_one_sale] = combined_df_currency_1[last_30_days_sales].apply(lambda x: Panda_Helper.days_in_last_x_with_at_least_y_sales(x, 31, 1))
                combined_df_currency_1[last_30_days_sales] = combined_df_currency_1[last_30_days_sales].apply(lambda x: Panda_Helper.total_sales_in_the_past(x))
    ...
    def create_list_of_bot_accounts():
        ...
        json_files = File_Handler.get_all_paths_of_type_of_orders("SELL", "CANCELLED", "ROLLING")
            ...
            with open(path, 'r', encoding='utf-8') as orders_in_string_file:
                for line_index, line in enumerate(orders_in_string_file):
                    try:
                        ...
                        temp_order = Order_Factory_GU.string_to_object(line_json)
        ...
        user_info_path = File_Handler.get_base_path("user_info_dic")
    ...
    def update_today_filled_sales(order_list, today_filled_order_timeline_dic):
        ...
        for order in order_list:
            ...
            updated_timestamp_str = Safe_Datetime_Converter.datetime_to_string(order.updated_timestamp)
    ...
    def update_base_line_filled_order_timeline(order_list, previous_filled_orders_dic):
        ...
        for order in order_list:
            ...
            timestamp_str = Safe_Datetime_Converter.datetime_to_string(order.updated_timestamp)
    ...
    ###### Creating Winrate Info ###########################################################################

    @staticmethod
    def update_win_rate_month_overview(win_rate_list):

        gods_unchained_updated_assets_dic = Gods_Unchained_Scrapper.get_all_gods_unchained_assets()
        ...
        user_ranking_dic = File_IO_Helper.load_user_rank_dic()

        #for user_name, ranking_dic in file_user_ranking_dic.items():


        last_friday = Data_Creator.determine_last_friday()
        new_win_rate_list = []
        year_month_to_load_dic = {}

        for win_rate_entry in win_rate_list:
            user_str = str(win_rate_entry.user_id)
            ...
            win_rate_entry.user_rank = user_rank
            new_win_rate_list.append(win_rate_entry)

            year_month = "-".join(win_rate_entry.day_finished.split("-")[0:2])
            year_month_to_load_dic[year_month] = True

        year_month_dic = {}
        months_overview_folder_path = File_Handler.get_base_path("months_overview_folder")
        ...
        for win_rate_entry in new_win_rate_list:
            for card_id in win_rate_entry.card_list:
                name = id_to_name_dic[card_id]

                year_month = "-".join(win_rate_entry.day_finished.split("-")[0:2])
                win_rate_dic = year_month_dic[year_month]
                win_rate_entry_dic = win_rate_dic[name]

                if win_rate_entry.day_finished in win_rate_entry_dic:
                    day_entry = win_rate_entry_dic[win_rate_entry.day_finished]
                else:
                    day_entry = {}
                ...
                win_rate_entry_dic[win_rate_entry.day_finished] = day_entry
                win_rate_dic[name] = win_rate_entry_dic
                year_month_dic[year_month] = win_rate_dic


            unique_card_dic = {card_id: True for card_id in win_rate_entry.card_list}
    ...
    def create_win_rate_info():
        ...
        total_win_last_30_rate_path = File_Handler.get_base_path("last_30_days_total_win_rate_file")
        mythic_win_rate_last_30_path = File_Handler.get_base_path("last_30_days_mythic_win_rate_file")
        ...
        days_in_last_4_weekend_rank = Data_Creator.determine_days_in_last_4_weekend_rank()
        ...
        total_win_rate_4_weekend_path = File_Handler.get_base_path("last_4_weekend_rank_total_win_rate_file")
        mythic_win_rate_4_weekend_path = File_Handler.get_base_path("last_4_weekend_rank_mythic_win_rate_file")
    ...
    ###### Creating Sales / Price /Price Change Info #######################################################
            ...
            for quality in quality_list:
                ...
                for card_name, price_change_dic in current_price_change_dic.items():
                    if len(price_change_dic) > 0:
                        ...
                        x_array = numpy.asarray(time_counter_list_unix).reshape(-1, 1)
    ...
    def create_filled_orders_sales_timeline(filled_order_timeline_dic):
        ...
        last_info_update_path = File_Handler.get_base_path("last_info_update")
        ...
        if today_not_created:
            ...
            filled_order_timeline_dic = Data_Creator.determine_all_needed_dics(filled_order_timeline_dic, 30)
            ...
            last_30_days_sales = File_Handler.get_base_path("last_30_days_sales")
            last_30_days_price = File_Handler.get_base_path("last_30_days_price")
            last_30_days_price_change = File_Handler.get_base_path("last_30_days_price_change")
            ...
            last_week_sales_path = File_Handler.get_base_path("last_7_days_sales")
            last_week_price_path = File_Handler.get_base_path("last_7_days_price")
            last_week_price_change_path = File_Handler.get_base_path("last_7_days_price_change")
    ...
    def determine_all_needed_dics(filled_order_timeline_dic, max_number_days):
        ...
        loaded_dics = File_IO_Helper.load_filled_order_timeline_dic(to_load_list)

        for year_month, filled_order_dic in loaded_dics.items():
            filled_order_timeline_dic[year_month] = filled_order_dic

        combined_timeline_dic = Data_Creator.create_combined_timeline_dic(filled_order_timeline_dic)
    ...
    def recreate_base_line_updated_active_orders():
        ...
        json_files_filled = File_Handler.get_all_paths_of_get_type_and_status_orders("SELL", "FILLED")
        json_files_filled = [path for path in json_files_filled if os.path.isfile(path)]
        filled_order_id_set = set(Order_Administrator_GU.get_all_order_id_for_type_order(json_files_filled))

        print("Loading Inactive Order IDs")
        json_files_inactive = File_Handler.get_all_paths_of_get_type_and_status_orders("SELL", "INACTIVE")
        json_files_inactive = [path for path in json_files_inactive if os.path.isfile(path)]
        inactive_order_id_set = set(Order_Administrator_GU.get_all_order_id_for_type_order(json_files_inactive))

        print("Loading Expired Order IDs")
        json_files_expired = File_Handler.get_all_paths_of_get_type_and_status_orders("SELL", "EXPIRED")
        json_files_expired = [path for path in json_files_expired if os.path.isfile(path)]
        expired_order_id_set = set(Order_Administrator_GU.get_all_order_id_for_type_order(json_files_expired))

        print("Loading Cancelled Order IDs")
        json_files_cancelled = File_Handler.get_all_paths_of_get_type_and_status_orders("SELL", "CANCELLED")
        json_files_cancelled = [path for path in json_files_cancelled if os.path.isfile(path)]
        cancelled_order_id_set = set(Order_Administrator_GU.get_all_order_id_for_type_order(json_files_cancelled))

        print("Creating Active Order Rolling DF")
        json_files_active = File_Handler.get_all_paths_of_get_type_and_status_orders("SELL", "ACTIVE")
        json_files_active = [path for path in json_files_active if os.path.isfile(path)]
        active_orders_df = Order_Administrator_GU.create_df_from_path_list(json_files_active)
        ...
        Order_Administrator_GU.write_updated_active_sell_order_df_to_file(final_active_orders_filtered_df)
    ...
    def recreate_base_line_filled_order_timeline():
        ...
        json_files = File_Handler.get_all_paths_of_type_of_orders("SELL", "FILLED", "ROLLING")
        ...
        for index, file_path in enumerate(json_files):
            ...
            order_list = Order_Administrator_GU.create_order_list_from_path(file_path)
            order_list = [x for x in order_list if x.card_name != None]

            year_month_list = Data_Creator.determine_year_month_orders_downloaded(order_list)
            previous_filled_orders_dic = File_IO_Helper.load_filled_order_timeline_dic(year_month_list)
            new_filled_orders_dic = Data_Creator.update_base_line_filled_order_timeline(order_list, previous_filled_orders_dic)
            for year_month, filled_order_dic in new_filled_orders_dic.items():
                combined_dic[year_month] = filled_order_dic
            File_IO_Helper.writing_filled_order_timeline_path_to_file(new_filled_orders_dic)

            today_filled_order_timeline_dic = File_IO_Helper.load_today_filled_order_timeline_dic()
            new_today_filled_order_timeline_dic = Data_Creator.update_today_filled_sales(order_list, today_filled_order_timeline_dic)
            File_IO_Helper.writing_today_filled_order_timeline_dic_to_file(new_today_filled_order_timeline_dic)
            Data_Creator.create_last_24_hours_filled_orders_sales_timeline(today_filled_order_timeline_dic)

        Data_Creator.create_filled_orders_sales_timeline(combined_dic)
    ...
    def recreate_base_line_win_rate():
        ...
        card_win_rate_to_be_processed_file_path  = File_Handler.get_base_path("card_win_rate_to_be_processed")
        lines_in_win_rate_to_be_processed_file = File_IO_Helper.get_lines_in_file(card_win_rate_to_be_processed_file_path)
        ...
        while not_caught_up_to_now:
            ...
            part_win_rate_list = Win_Rate_Administrator.create_limited_win_rate_to_be_processed_list(start_position, end_position)

            print("Creating Month Overview")
            Data_Creator.update_win_rate_month_overview(part_win_rate_list)
    ...
    @staticmethod
    def recreate_user_dic():

        connection_manager = Connection_Manager()

        try:
            guwp = Gods_Unchained_Winrate_Poller()
            guwp._recreate_user_dic()

        except (Response_Error, Request_Error, TooManyAPICalls, Internal_Server_Error) as custom_errors:
            print("")
            print(custom_errors)
            connection_manager.switch_ip()
            Data_Creator.recreate_user_dic()
