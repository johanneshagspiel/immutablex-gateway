...
class ConnectionManager:
    """
    A class to automate switching between NordVPN servers
    """

    def __init__(self):
        """
        The constructor of the ConnectionManager class
        """

        nordvpn_server_time_and_names_path = FileHandler.get_base_path("nordvpn_server_time_and_names")
        ...
        self._weight_list = [float(unnormalized_weight)/sum(unnormalized_weight_list) for unnormalized_weight
                             in unnormalized_weight_list]

    def switch_ip(self, not_first_time=True):
        """
        A method to switch to a different IP server
        :param not_first_time: Boolean whether this is the first time a server is switched. If it is not the first time,
        additionally information is stored how long the previous connection has lasted
        :return: None
        """
        ...
        server_up_time_path = FileHandler.get_base_path("server_up_time")
        nordvpn_server_time_and_names_path = FileHandler.get_base_path("nordvpn_server_time_and_names")
        ...
        if not_first_time:
            ...
            prev_start_time_timestamp = SafeDatetimeConverter.string_to_datetime(prev_start_time)
            ...
            with open(nordvpn_server_time_and_names_path, 'w', encoding='utf-8') as nordvpn_server_time_and_names_file:
                json.dump(new_server_time_and_name_list, nordvpn_server_time_and_names_file,
