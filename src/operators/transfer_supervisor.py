...
class Transfer_Supervisor():
    ...
    def supervise_card(self, inventory_entry, all_cards_available, currency_prices_dic, competition_price_dic):
        ...
        price_floor_crypto_str = Number_Converter.quantize_number_str((price_floor_multiplier * inventory_entry.purchase_price_euro) / currency_prices_dic[inventory_entry.sale_currency])
        ...
        no_other_offers_price_str = Number_Converter.quantize_number_str((1.25 * inventory_entry.purchase_price_euro) / currency_prices_dic[inventory_entry.sale_currency])
        ...
        if isinstance(all_cards_available, DataFrame):
            ...
            price_crypto_float_list = np.vectorize(lambda x, y: 1.05 * Number_Converter.get_float_from_quantity_decimal(x, y))(all_cards_available["quantity"], all_cards_available["decimals"])
            ...
            all_cards_available_list = [(x["sell"]["data"]["token_id"], Number_Converter.get_float_from_quantity_decimal(x["buy"]["data"]["quantity"], x["buy"]["data"]["decimals"]), x["order_id"]) for x in all_cards_available]
        ...
        if inventory_entry.token_id in token_id_list:
            ...
            current_price_str = str(Number_Converter.quantize_number_str(current_entry[1]))
        ...
        if len(missing_active_orders_list) > 0:
            ...
            competition_price_dic, additional_info_list = Competition_Handler.add_missing_orders_to_competition_file(missing_active_orders_list, competition_price_dic, inventory_entry)
        ...
        if len(sorted_other_offers_adjusted) > 0:
            ...
            for price_position, (token_id, price_float) in enumerate(sorted_other_offers_adjusted):
                ...
                if price_float > price_floor_crypto_float:
                    ...
                    price_str = Number_Converter.quantize_number_str(price_float)
                    next_cheapest_price_str = Number_Converter.get_next_cheapest_number(price_str)
